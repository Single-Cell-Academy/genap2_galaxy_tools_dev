...
This is an important final step of setting up the container. We need to enable log rotation, otherwise error logs can clog the system and create problems for other users containers.

In your /etc/galaxy/galaxy/yml search for the following option and uncomment.
...
Afterward, we have to create a file called /etc/logrotate.d/galaxy and insert the following code:
...
If you recently destroyed your galaxy app but saved your config files beforehand, you can simply copy paste these files above the defaults and basically recreate all of your previous settings. If your config files are in your home folder under /galaxy_config_backup. Run these commands:
...
In case the Galaxy app returns a spawn error when trying to restart using the following command: 